...
public partial class Utf8ComparerTest
{
    ...
    public void Default_EqualsAndCompare(string testText)
    {
        ...
        var x = Utf8String.DangerousFromUtf8([.. Encoding.UTF8.GetBytes(testText)]);
        Assert.True(comparer.Equals(x, x));

        {
            // length differs
            var y = Utf8String.DangerousFromUtf8([.. Encoding.UTF8.GetBytes(testText + ' ')]);
        ...
        {
            var y = Utf8String.DangerousFromUtf8([.. Encoding.UTF8.GetBytes(
                testText[0..i] +
